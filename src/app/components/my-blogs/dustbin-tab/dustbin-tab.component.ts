...
@Component({
  selector: 'app-dustbin-tab',
  templateUrl: './dustbin-tab.component.html',
  styleUrls: ['./dustbin-tab.component.scss'],
})
export class DustbinTabComponent extends BlogTableComponent {
  @ViewChild('tabHeader') tabHeader!: ElementRef;
  @ViewChild('tableContainer') tableContainer!: ElementRef;
  @ViewChild('tabFooter') tabFooter!: ElementRef;

  state: number = STATE.DELETED;
  canEdit: boolean = false;
  canRestore: boolean = true;
  canDelete: boolean = true;
  activeName: string = 'dustbin';
  userId: number = -1;
  ...
  override ngOnInit(): void {
    ...
    this.blogService.$updateDustbinTab
  ...
  getColDefs(): TableColumn[] {
    return [
      ...
      },
      {
        ...
        actions: {
          restore: (blog: Blog) => this.onRestore(blog),

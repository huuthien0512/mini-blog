...
@Component({
  ...
  styleUrls: ['./draft-tab.component.scss'],
})
export class DraftTabComponent extends BlogTableComponent {
  @ViewChild('tabHeader') tabHeader!: ElementRef;
  @ViewChild('tableContainer') tableContainer!: ElementRef;
  @ViewChild('tabFooter') tabFooter!: ElementRef;

  state: number = STATE.DRAFTED;
  canEdit: boolean = true;
  canRestore: boolean = false;
  canDelete: boolean = true;
  activeName: string = 'draft';
  userId: number = -1;
  ...
  override ngOnInit(): void {
    ...
    super.ngOnInit();
    this.blogService.$updateDraftTab
